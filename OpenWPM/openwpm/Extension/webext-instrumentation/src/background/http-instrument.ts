...
export class HttpInstrument {
  ...
  public run(crawlID, saveContentOption: SaveContentOption) {
    ...
    function notify(data) {
      ...
      if (data.message == "finish-button-click" || data.message == "about-to-button-click") {
        let message = data.message;
        console.log(`background script success! Received ${message}. It is of type ${typeof message}`);
        if (message == "finish-button-click") {
          blockRequest = false;
          console.log(`${message} Set blockRequest to false`);
        } else if (message == "about-to-button-click") {
          blockRequest = true;
          console.log(`${message} Set blockRequest to true`);
        }
      } else if (data.message == "triggered_func") {
        triggeredFuncData.push(data.data);
        console.log(`add new data to triggeredFuncData`);
      } else if (data.message == "clear_triggered_func_data") {
        triggeredFuncData = [];
        usedString = [];
        console.log(`clear_triggered_func_data`);

      } else if (data.message == "send_triggered_func_data") {
        console.log(`send_triggered_func_data`);

        triggeredFuncData = triggeredFuncData.filter(data => usedString.includes(data.used_string));

        for (let obj of triggeredFuncData) {
          obj.ith_form = ith_form;
        }

        fetch('http://localhost:3050/triggered_func', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify(triggeredFuncData),
        })
          .then(data => console.log(data));
      } else if (data.message == 'send_used_string') {
        usedString = data.data;
        ith_form = data.ith_form
      }
    }
    ...
    // const requestStemsFromOpenWPMInitList = details => {
    //   return (
    ...
    this.onBeforeSendHeadersListener = (details) => {
      ...
      if (blockRequest && (this.exceptionForTestcaseAndDataCollection(details.url))) {
        return { cancel: true };
      }
      return {};
    };
    // this.onBeforeSendHeadersListener = async details => {
    //   // Ignore requests made by extensions
    //   // if (requestStemsFromExtension(details) || requestStemsFromOpenWPMInitList(details)) {
    //   if (requestStemsFromExtension(details)) {
    //     return {};
    //   }
    //   const pendingRequest = this.getPendingRequest(details.requestId);
    ...
    //   // let response = await fetch('http://localhost:3050/is-blocking');
    //   // let data: { isBlocking: boolean } = await response.json();
    //   // let isBlocking = data.isBlocking;
  ...
  private isToLocalServer(url: String): Boolean {
    return url.startsWith("http://localhost:3050");
  }

  private exceptionForTestcaseAndDataCollection(url: string): Boolean {
    return (!this.isToLocalServer(escapeUrl(url)) || escapeUrl(url).startsWith('http://localhost:3050/?'));
  }
  ...
  private async onBeforeSendHeadersHandler(
    ...
    // instrumented code
    console.log(escapeUrl(details.url));
    if (isBlocking && (this.exceptionForTestcaseAndDataCollection(details.url))) {
      let openAPI = {
        url: update.url,
        post_body: update.post_body,
        post_body_raw: update.post_body_raw,
        headers: update.headers,
        method: update.method
      };

      let testData = { openAPI: JSON.stringify(openAPI), page_url: update.triggering_origin, is_successful_submit: true, ith_form };

      // update.url   // this is the target url
      // update.post_body
      // update.post_body_raw
      // update.headers
      // update.method   -> I only want POST and GET


      fetch('http://localhost:3050/on_submit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
