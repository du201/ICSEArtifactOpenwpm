...
class RegexSolver:
    ...
    def _len_set(self, r):
        ...
        if ty == regex_parser.EMPTY:
            ...
            '''
            elif ty == regex_parser.NAMEDGROUP:
                return self._len_set(r[2])
            
            elif ty == regex_parser.NAMEDBACKREF:
                name = r[3]
                return self._len_set(self.named_groups[name])
            '''
    ...
    def _sat_expr(self, x, r, i, l):
        ...
        if ty == regex_parser.EMPTY:
            ...
            return expr #        '''elif ty == regex_parser.NAMEDGROUP:
        elif ty == regex_parser.BACKREF:
            idx = r[1] - 1
            expr = z3.BoolVal(False)
...
class RegexSolver:
    ...
    def _set_sum(self, a, b):
        ...
        for i in a:
            for j in b:
                if self.maxLength == None or (i + j) <= self.maxLength:
                    s.add(i + j)
        return s
    ...
    def _sat_expr(self, x, r, i, l=None):
        if l not in self._len_set(r):
            return z3.BoolVal(False)
        if self.maxLength != None and i + l > self.maxLength:
            return z3.BoolVal(False)
        ...
        if ty == regex_parser.EMPTY:
            ...
            inner = r[1]
            if (self.maxLength == None or i + l < self.maxLength) and (self.minLength == None or i + l > self.minLength):
                return self._sat_expr(x, inner, i, l)
            else:
                return z3.BoolVal(False)
    ...
    def sat_expr(self):
        expr = self._sat_expr(self.x, self.regex, 0, self.maxLength)
        return z3.simplify(expr)
'''