...
[EMPTY, CHAR, DOT, STAR, BAR, NAMEDGROUP, CONCAT, GROUP, BACKREF, CARET, DOLLAR] = range(11)
CHARSET = string.ascii_lowercase + string.ascii_uppercase + string.digits + " :/'!" + ".*+?|()[]^-{},\\" # unicode / ascii

character_classes = {
    's' : " ",
    'w' : string.ascii_lowercase + string.ascii_uppercase + string.digits,
    'd' : "0123456789", # unicode / ascii
}
...
class RegexLexer:
    ...
    t_LDASHBRACKET, t_RDASHBRACKET = r"\[-", r"-\]"
...
class RegexParser:
    ...
    '''
    def p_factor_namedgroup(self, p):
        """factor : NAMEDGROUP LTRI string RTRI regex RPAREN"""
        self.named_groups[p[3]] = p[5]
        p[0] = (NAMEDGROUP, p[3], p[5]) # 0 is a placeholder
    '''
    def p_groupname_base(self, p):
        """string : NON_META_CHAR"""
        p[0] = p[1]
    def p_groupname_recursive(self, p):
        """string : NON_META_CHAR string"""
        p[0] = p[1] + p[2]
    ...
    def p_set_start_dash(self, p):
        """set : LDASHBRACKET set_items_parent RBRACKET"""
        p[0]=p[2].union('[')
    def p_set_end_dash(self, p):
        """set : LBRACKET set_items_parent RDASHBRACKET"""
        p[0]=p[2].union(']')
    def p_set_positive(self, p):
        """set : LBRACKET set_items_parent RBRACKET"""
        p[0] = p[2]

    
    def p_set_items_parent_no_dash(self, p):
        """set_items_parent : set_items"""
        p[0] = p[1]
    '''
    def p_set_items_parent_start_dash(self, p):
        """set_items_parent : DASH set_items"""
        p[0] = p[2].union(p[1])
    def p_set_items_parent_end_dash(self, p):
        """set_items_parent : set_items DASH"""
        p[0] = p[1].union(p[2])
    '''
    def p_set_negative(self, p):
        """set : LBRACKET CARET set_items_parent RBRACKET"""
